...
wd=/90daydata/bull_age/liu.yang/imputation/pangenie_cdcb/3.pan_all/1.imputation_eval/check
cd $wd
...
cat > extract_SV.awk << 'EOF'
BEGIN{FS=OFS="\t"}
$1 ~ "#"{print;next}
{split($3,a,/:|-|_/);if(a[4] == "DEL" || a[4] == "INS" || a[4] == "COMPLEX") {
$4="A";$5="G";
print}
}
EOF
...
ls $wd/../imp.pang.*.vcf.gz |
    ...
    sbatch -A bull_scr \
        ...
        --job-name=split_submit.$comp \
        --output=logs/$comp.split_submit.%A_%a.out \
        --wrap="
bash run.split_submit.sh $comp $wd $wd/val.sv.vcf.gz
                "
    done
