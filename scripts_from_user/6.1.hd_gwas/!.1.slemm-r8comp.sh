...
zcat /90daydata/bull_age/liu.yang/imputation/pangenie_cdcb/3.pan_all/All2_BTAN_WGS-var.vcf.gz | 
grep  -v  "#" | cut -f 3 |  awk '{split($1,a,/-|_/);print a[2]}' | sort | uniq -c




zcat cdcb173wgs-var.vcf.gz | grep  -v  "#" | cut -f 3 |  awk '{split($1,a,/-|_/);print a[2]}' | sort | uniq -c
  6323 COMPLEX
  15190 DEL
  26489 INS
10636245 SNP
...
cat cdcb173WGSsv_HolrePan.filter.r8.pvar | grep  -v  "#" | cut -f 3 |  awk '{split($1,a,/-|_/);print a[2]}' | sort | uniq -c
   6222 COMPLEX
  15054 DEL
  26073 INS
# data
