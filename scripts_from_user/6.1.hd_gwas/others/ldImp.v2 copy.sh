...
panref_vcf=/90daydata/bull_age/liu.yang/imputation/hd_gwas/sv_gwas/Holsteinandrelated.pangenie-var.vcf.gz
...
ls svwgssnps/logs/ | cut -d. -f2 | sort | uniq -c
ls svldsnps/logs/ | cut -d. -f2 | sort | uniq -c
ls svldsnps/logs/ | grep chr28 | cut -d. -f3 | sort -n | uniq -c  | awk '$1 != 2 || $2 != NR-1 ' | head
...
cd /90daydata/bull_age/liu.yang/imputation/hd_gwas/genotypes/$ref_type/
seq 24 29 | while read chr; do
    sbatch -A bull_age \
        --job-name=chr$chr.$ref_type \
        --cpus-per-task=16 \
