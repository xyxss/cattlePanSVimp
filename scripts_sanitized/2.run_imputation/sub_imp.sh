...
panref_prefix=${PROJECT_ROOT}/sbcp_imp/genotypes/ref_Holre_pangenie-var/ref_output
snp_vcf=${PROJECT_ROOT}/hd_gwas/genotypes/svwgssnps/snp_input.$chr.maf01.vcf.gz

work_dir=${PROJECT_ROOT}/hd_gwas/genotypes/1.svImp_wgs
...
## ${PROJECT_ROOT}/hd_gwas/genotypes/imput.array.sbatch.sh

cat > ${PROJECT_ROOT}/hd_gwas/genotypes/imput.array.sbatch.sh << 'EOF2'
#!/bin/bash
set -eu
...
cat > ${PROJECT_ROOT}/hd_gwas/genotypes/merge.array.sbatch.sh << 'EOF3'
#!/bin/bash
set -eu
chr=$1
work_dir=$2
...
cat > ${PROJECT_ROOT}/hd_gwas/genotypes/merge.chr.sbatch.sh << 'EOF4'
#!/bin/bash
set -eu
out_prefix=$1
work_dir=$2
...
seq 1 29 | while read chr; do
    ...
    sbatch \
        ...
        --output=logs/wgsArray.chr$chr.%a.out \
        --wrap "
        bash ${PROJECT_ROOT}/hd_gwas/genotypes/imput.array.sbatch.sh $chr $work_dir $snp_vcf
        " | awk '{print $4}' | xargs -I {} \
        sbatch -A ${SLURM_ACCOUNT} --dependency=afterok:{} \
        --job-name=chk$chr.merge \
        --cpus-per-task=$nthreads \
        --error=logs/merge.chk$chr.err \
        --output=logs/merge.chk$chr.out \
        --wrap="
        bash ${PROJECT_ROOT}/hd_gwas/genotypes/merge.array.sbatch.sh $chr $work_dir
        " 
        done 
    ...
    sbatch -A ${SLURM_ACCOUNT} \
        ...
        --wrap="
        bash ${PROJECT_ROOT}/hd_gwas/genotypes/merge.chr.sbatch.sh \
        $work_dir/umd50ksWGS_HolrePan $work_dir
        "


## ${PROJECT_ROOT}/hd_gwas/genotypes/merge.array.sbatch.sh
####
nthreads=8
work_dir=${PROJECT_ROOT}/hd_gwas/genotypes/3.svImp_cCattle
mkdir -p $work_dir/tmp/chunk $work_dir/logs
#bcftools query -l ${PROJECT_ROOT}/cGTEx_data/data/lfang2/Imputeded_SNPs/Chr1.all-beagle.vcf.gz  | wc -l
...
#snp_vcf=${PROJECT_ROOT}/hd_gwas/genotypes/svwgssnps/snp_input.$chr.maf01.vcf.gz
seq 1 29 | while read chr; do
    snp_vcf=${PROJECT_ROOT}/cGTEx_data/data/lfang2/Imputeded_SNPs/Chr$chr.all-beagle.vcf.gz
    sbatch \
        -A ${SLURM_ACCOUNT} \
        --array=0-72 \
    ...
    --output=logs/mergeChr.out \
    --wrap="
    bash ${PROJECT_ROOT}/hd_gwas/genotypes/merge.chr.sbatch.sh $work_dir/cattlegtex_HolrePan $work_dir
    "



work_dir=${PROJECT_ROOT}/hd_gwas/genotypes/2.svImp_ld
snp_vcf=${PROJECT_ROOT}/hd_gwas/hol.chip.maf01.reheader.vcf.gz
mkdir -p $work_dir/tmp/chunk $work_dir/logs
cd $work_dir
...
seq 13 29 | while read chr; do
    ...
    sbatch -A ${SLURM_ACCOUNT} \
        ...
        --output=logs/mergeChr.out \
        --wrap="
        bash ${PROJECT_ROOT}/hd_gwas/genotypes/merge.chr.sbatch.sh $work_dir/umd50ksLD_HolrePan $work_dir
        "
