...
cd ${PROJECT_ROOT}/hd_gwas/1.svImpCompWGS
...
cd ${PROJECT_ROOT}/pangenie_cdcb/3.pan_all/
...
# phenotype
sed 's/\t/,/g' ${PROJECT_ROOT}/pangenie_cdcb/plink_gwas/ph.hol.traits > ph.hol.traits.csv

# data

awk 'NR==FNR{a[$1,$2,$4,$5]=$3;next} {if(a[$1,$2,$4,$5] != "") {print $3,"===",a[$1,$2,$4,$5]}}' \
    ../0.svImp_r8/umd50ksWGS_HolrePan.filter.r8 \
    <(zcat ${PROJECT_ROOT}/pangenie_cdcb/3.pan_all/All2_BTAN_WGS-var.vcf.gz) |
...
sbatch -A ${SLURM_ACCOUNT} --partition ${SLURM_PARTITION} \
        ...
        --time "14-00:00:00" \
        --wrap="
bash ${PROJECT_ROOT}/hd_gwas/genotypes/slemm_gwas.sh \
...
slemm_path=${SCRATCH_DIR}/software/slemm-v0.90.1-x86_64-linux/
