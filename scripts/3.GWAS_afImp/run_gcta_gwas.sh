...
cat > ${PROJECT_ROOT}/hd_gwas/genotypes/gcta_grm.sh <<'EOF'
pre_gdata=$1
nall=$2
nc=$3
...
seq 1 $nall | while read ii; do
    sbatch -A ${SLURM_ACCOUNT} \
        ...
        --time "1-00:00:00" \
        --wrap="
bash ${PROJECT_ROOT}/hd_gwas/genotypes/gcta_grm.sh \
    $pre_gdata $nall $ii
"
done
...
#cat > ${PROJECT_ROOT}/hd_gwas/genotypes/gcta_fastmlm.sh <<"EOF"
pre_gdata=$1
out_pre=$2
trait=$3
cov=$4
...
pc=10
grm=${PROJECT_ROOT}/hd_gwas/hol.chip.sp_grm
...
sbatch -A ${SLURM_ACCOUNT} --partition ${SLURM_PARTITION} \
        ...
        --wrap="
bash ${PROJECT_ROOT}/hd_gwas/genotypes/gcta_fastmlm.sh \
