...
# Cohort sampling seed. The published analysis used SAMPLING_SEED=1; changing it
# produces a different set of benchmark animals and different accuracy estimates.
SAMPLING_SEED=${SAMPLING_SEED:-1}

# shuf with a seeded keystream, so cohort membership is reproducible across runs
# and machines. Plain `shuf` reseeds from the OS each invocation.
function seeded_shuf () {
...
cat holPub.samples.group | awk -F "\t" '$2 == "Holstein" {print $1}' | seeded_shuf -n 750 > Holstein750
#963
seeded_shuf -n 250 Holstein > Holstein250
cat holPub.samples.group | awk -F "\t" '$2 ~ "Holstein-X-Jersey" {print $1}' | seeded_shuf -n 250 > HolsteinRelated250
cat holPub.samples.group | awk -F "\t" '$2 == "Jersey" {print $1}' | seeded_shuf -n 250 > Jersey250
...
mc_vcf=${PROJECT_ROOT}/minigraph-cactus/hol-pg2hic-2024-05-22/hol-pg2hic-2024-05-22.anno_biallelic.vcf.gz
